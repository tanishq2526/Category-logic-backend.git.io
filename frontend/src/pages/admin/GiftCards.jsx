  ...
  useEffect(() => {
    if (editingCard) {
      setFormData({
        ...
        expiryDate: editingCard.expiryDate ? new Date(editingCard.expiryDate).toISOString().split('T')[0] : "",
    ...
    <div className="gc-modal-backdrop">
      <div className="gc-modal">
        <div className="gc-modal-header">
          ...
          <button onClick={onClose} className="gc-modal-close-btn">✕</button>
        </div>

        <div className="gc-modal-body">
          <div className="gc-modal-form-group">
            <label className="gc-form-label">Assigned Person Name *</label>
            <input type="text" name="assignedTo" value={formData.assignedTo} onChange={handleChange} placeholder="Enter assigned person name" className="gc-form-input" />
          </div>

          <div className="gc-modal-form-group">
            <label className="gc-form-label">Gifted By Person Name *</label>
            <input type="text" name="giftedBy" value={formData.giftedBy} onChange={handleChange} placeholder="Enter gifted by person name" className="gc-form-input" />
          </div>

          <div className="gc-form-grid-2">
            <div className="gc-modal-form-group">
              <label className="gc-form-label">Code *</label>
              <input type="text" name="code" value={formData.code} onChange={handleChange} placeholder="GC-2024-001" className="gc-form-input" />
            </div>
            <div className="gc-modal-form-group">
              <label className="gc-form-label">Value / Discount *</label>
              <input type="number" name="value" value={formData.value} onChange={handleChange} placeholder="500" className="gc-form-input" />
            </div>
          </div>
          ...
          <div className="gc-form-grid-2">
            <div className="gc-modal-form-group">
              ...
              <input type="date" name="expiryDate" value={formData.expiryDate} onChange={handleChange} className="gc-form-input" />
            </div>
            <div className="gc-modal-form-group">
              <label className="gc-form-label">Status</label>
              <select name="status" value={formData.status} onChange={handleChange} className="gc-form-select">
          ...
          <div className="gc-modal-form-group">
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Enter gift card description or message" className="gc-form-textarea" />
          </div>
        </div>

        <div className="gc-modal-footer">
          <button onClick={onClose} className="gc-btn-cancel">Cancel</button>
          <button onClick={handleSave} className="gc-btn-save">Save</button>
        </div>
      </div>
    </div>
  );
...
const GiftCardGridCard = ({ card, onEdit, onDelete }) => {
  ...
  const getStatusClassName = (status) => {
    switch (status) {
      ...
      case "used":
      case "inactive": return "gc-status-used";
      default: return "gc-status-active";
    }
  };
  ...
  return (
    <div className="gc-card">
      <div className="gc-card-header">
        ...
        <span className={`gc-status-badge ${getStatusClassName(displayStatus)}`}>
      ...
      <div className="gc-card-field">
        ...
        <p className="gc-card-field-value">{new Date(card.expiryDate).toLocaleDateString()}</p>
      </div>
      ...
      <div className="gc-card-actions">
        <button onClick={() => onEdit(card)} className="gc-btn-action gc-btn-edit">
          <EditIcon /> Edit
        </button>
        <button onClick={() => onDelete(card._id)} className="gc-btn-action gc-btn-delete">
          <DeleteIcon /> Delete
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      <div className="gc-results-info">
        Showing {giftCards.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} to{" "}
        {Math.min(currentPage * itemsPerPage, totalCards)} of {totalCards} gift cards
      </div>
